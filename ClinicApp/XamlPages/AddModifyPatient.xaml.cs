    ...
    public partial class AddModifyPatient : Page
    {
        ...
        private async void btnSubmit_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                {
                    status = false;
                    messageBuilder.Append("Дата рождения не может быть больше текущей даты.\n");
                }
                else if (dateOfBirth.Value <= SqlServerConstants.MinSqlServerDate)
                {
                    status = false;
                    messageBuilder.Append("Дата рождения не может быть меньше 01.01.1753");
                }
                #endregion
